...
仅仅备份 Git 仓库还不够。执行一键安装前，必须在本仓库的 **Releases** 页面创建至少一个 GitHub Release，例如 tag 为 `v0.9.4`，并上传目标服务器架构对应的 ZIP 文件。当前脚本不携带 GitHub Token，因此 `master` 分支和 Release 资产必须允许公开下载；如果仓库是私有仓库，需要另外增加鉴权逻辑。
...
2. 在 `zcw666806/XrayR-release` 仓库创建 GitHub Release，例如 `v0.9.4`，上传 `XrayR-linux-64.zip`。
...
您已经建立 `v0.9.5` Release 后，可以在 `zcw666806/XrayR-release` 的 **Actions → Build and upload XrayR release assets → Run workflow** 中填写：
...
XrayR update 0.9.4    # 安装 Release tag v0.9.4
```
